...
# ╔═╡ 88aa329a-c476-11f0-aced-d7a3532d679f
begin
	using CSV
	using CairoMakie
	using DataFrames
...
# ╔═╡ 431f8555-6309-40cd-aa37-2d3e38b3e88b
md"""

# Transport in Glasgow using the Scottish Household Survey
[Graham Stark](graham.stark@northumbria.ac.uk)

"""

# ╔═╡ 8405e58f-724e-44b8-9cd9-4e1160e65227
md"""
Load 3 waves of household data and extract Glasgow Subset
"""

# ╔═╡ 8f902ad9-f724-4a26-aa91-96d8306a7767
begin

# change this 
const DIR="/mnt/data/"
# lookup table of LA names->codes
LANAMES = CSV.File(joinpath( DIR, "ScotBen", "artifacts", "augdata", "scottish-las-shs-ons-names.tab" ))|>DataFrame
rename!(strip,LANAMES)
LANAMES.lad_2017 = Symbol.(LANAMES.lad_2017)
...
function loadshs( dyear::Int )::DataFrame
    ...
    shs = CSV.File( fname; 
        missingstring=["NA","","-99","-98","-97","999998.0","999999.0"],normalizenames=true,
...
shs = create_shs( 2021:2022 )
dropmissing!( shs, :council )
# drop covid year
shs = shs[shs.datayear .!= 2020,:]
end

# ╔═╡ def416fd-e17b-4938-ab1f-642b10d7317f
LANAMES

# ╔═╡ d7ed6f2c-71f1-4221-be27-88f39b93487e
begin
# recode some stuff
scodefind( c::Union{AbstractString,Missing}; field=:lad_2017) = LANAMES[ LANAMES.shs_code .== c, field][1]

# shs.uniqidnew = parse.(Int,shs.uniqidnew)
shs.lad_2017 = scodefind.( shs.council,field=:lad_2017 )
shs.council_name = scodefind.( shs.council,field=:namev2 )
# glasgow subset
shsg = shs[shs.lad_2017.==:S12000049,:]

end;

# ╔═╡ 958206c4-fc8e-4995-a6da-7705d4820ea4
# sample by council
sort(countmap(shs.council_name))

# ╔═╡ b5384e16-d8ab-4116-96de-e50a872a7880
# sample by year
sort(countmap(shsg.datayear))

# ╔═╡ 660285cf-9b08-4969-8bb2-262025c627c2
summarystats(shsg.parkcost)
...
manifest_format = "2.0"
project_hash = "78539f328b4ddfa9f521879ff51a2d7d67f15e8e"
...
git-tree-sha1 = "d3607b2cdabf222fa0cc0c01cc94f2bc4dd6be19"
...
version = "0.7.9"
    ...
    RegressionTablesTypstryExt = "Typstry"
