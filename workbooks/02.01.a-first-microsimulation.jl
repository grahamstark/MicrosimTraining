...
begin
 using DataFrames,CSV,Downloads,Plots,StatsPlots, Formatting, PovertyAndInequalityMeasures, PlutoUI
end

# ╔═╡ b576d212-a75c-11ee-1ed4-e35ed870cd2e
 md"""# IMA 2024 Julia Microsimulation Demo
...
# ╔═╡ f7ef5d9b-b32b-49f1-a8be-fa270a807b87
	...
	# we can graph here as before 
#	@df lcf scatter( :hhincome, :food_and_drink; group=:tenure )
...
begin
	...
	# holder for results from one household
...
# ╔═╡ 4d7118b1-3c62-4458-8f79-f460468a9103
md"""
...
# ╔═╡ 32e8e863-5672-4f34-8684-043803c1a662
...
# ╔═╡ 45c909e2-f7db-4120-8c5c-0f3c9bada330
md"""
## Initialise Systems
...
# ╔═╡ 3852c91c-f088-4760-b06e-a85edf530e1b
md"""
## Sample Weights
...
* We uprate by the [change in Nominal GDP between 2005 Q3 and 2023 Q3](https://www.ons.gov.uk/economy/grossdomesticproductgdp/timeseries/abmz/ukea). Note the slightly strange syntax for the multiplication: `!` means 'select all rows', then there's a list of all the columns we need to uprate, then `.*= INFLATION` is a `broadcast multiply` operation.

"""

# ╔═╡ 6fb11979-858b-4ccb-a0d9-7c1a936d66c9
begin
	const SAMPLE_SIZE = size(lcf)[1] # count of rows
	const NUM_HHLDS = 24_783_192
	const WEIGHT = NUM_HHLDS/SAMPLE_SIZE
	# https://www.ons.gov.uk/economy/grossdomesticproductgdp/timeseries/abmz/ukea
    const INFLATION = 678129/353999
...
# ╔═╡ 252ca385-d319-461a-ab9c-c8d1b7cf4a33
begin
	
	# res1 and res2 are DataFrames holding lists of results for a pre- and post- system run. See the next cells for more. Return a `NamedTuple` of summary results. 
	function analyse( res1::DataFrame, res2::DataFrame )::NamedTuple
		tax1 = sum(res1.tax .+ res1.ni .+ res1.vat)*52*WEIGHT
		tax2 = sum(res2.tax .+ res2.ni .+ res2.vat)*52*WEIGHT
		ben1 = sum(res1.benefit)*52*WEIGHT
		ben2 = sum(res2.benefit)*52*WEIGHT
		dben = ben2-ben1
		dtax = tax2-tax1
		ineq1 = make_inequality( res1, :weight, :net )
		ineq2 = make_inequality( res2, :weight, :net )
		dgini = ineq2.gini - ineq1.gini
	    dpalma = ineq2.palma - ineq1.palma
	    
		gainers = sum(res1[res2.net .> res1.net,:weight])
		losers = sum(res1[res2.net .< res1.net,:weight])
		nc = sum(res1[res2.net .== res1.net,:weight])

		gain = ineq2.deciles[:,4] .- ineq1.deciles[:,4]
		deciles = bar( string.(1:10), gain, xlabel="Decile", ylabel="£s pw", title="Gains/Losses By Decile", labels="Av Change £s pw" )

		i1 = copy(ineq1.deciles[:,1:2])
		i1 = vcat([0.0 0.0],i1)
		i2 = copy(ineq2.deciles[:,1:2])
		i2= vcat([0.0 0.0],i2)		
		lorenz = plot(i1[:,1],i1[:,2],
			labels="res1", 
			title="Lorenz Curve", 
			...
			xrange=(0,1), 
			yrange=(0,1))
		plot!(lorenz,i2[:,1],i2[:,2],labels="res2" )
		plot!(lorenz, 0:1, 0:1,labels="Equality" )
...
# ╔═╡ 628da3b8-bdce-4c30-b076-a4a28ae2517d
md"""
...
# ╔═╡ b2fb8ad2-29f8-4443-a406-0c9aa5bb9d45
md"""
...
# ╔═╡ 43c75919-761c-4a8c-bc88-0826650d6e6c
begin
	
end

# ╔═╡ 8f0e1ebc-bc1a-4b5d-be31-e9d74e0f7df8
md"""
tax allowance: $(@bind new_allowance NumberField(0:200:25000,default=10000)) (p.a.) 

income tax rate: $(@bind new_it_rate NumberField(0:1:75,default=25)) (%)

vat: $(@bind new_vat_rate NumberField(0:1:75,default=20)) (%)

extend vat? $(@bind new_extend_vat CheckBox(default=false)) 

payroll tax: $(@bind new_ni_rate NumberField(0:1:75,default=10)) (%)

benefit rate $(@bind new_benefit NumberField(0:5:250,default=100)) p.w.

"""

# ╔═╡ 95463e3e-0819-4085-80dc-f4372df1c744
begin
	sys1 = System( 10_000/52, 0.25, 0.10, 100.0, 0.20, false )
	sys2 = System( new_allowance/52, new_it_rate/100, new_ni_rate/100, new_benefit, new_vat_rate/100, new_extend_vat )
end

# ╔═╡ 9f9d7089-e51f-4e86-b9df-5184b8114086
begin
	function one_calc( sys :: System ) :: DataFrame 
		n = size(lcf)[1]
		out = DataFrame( gross=zeros(n), net=zeros(n), tax=zeros(n), ni=zeros(n), vat=zeros(n), benefit=zeros(n) )
		i = 0
		for hh in eachrow( lcf )
			i += 1
			res = calc_system( hh, sys )
			out.gross[i] = hh.hhincome
			out.net[i] = res.net
			out.tax[i] = res.tax
			out.ni[i] = res.ni
			out.vat[i] = res.vat
			out.benefit[i] = res.benefit
		end
		out.weight = fill(WEIGHT,n)
		return out
	end

	res1 = one_calc( sys1 )
	res2 = one_calc( sys2 )
	res = analyse( res1, res2 )
	""
end

# ╔═╡ 79928846-b8ae-4f8e-ae0b-e052533daf37
res.lorenz

# ╔═╡ 16782bf3-57f7-498b-a28b-764b3de2b48e
res.deciles

# ╔═╡ afd85b9b-a013-46de-9015-d2bfbc1c3569
begin
# new_vat_rate=0.2
# new_extend_vat=false
# new_benefit = 100
# new_it_rate = 0.35
# new_ni_rate = 0.1
# new_allowance=10_000
display( res )
end

# ╔═╡ 7fa9a68e-79d7-405c-beba-f8d05640d155
begin
	
function showchanges(sys1::System, sys2::System)
	
md"""
tax allowance pre: **£$(fc(sys1.allowance*52))p.a** post: **£$(fc(sys2.allowance*52))p.a**

income tax rate: pre: **$(fc(sys1.it_rate*100))%** post: **$(fc(sys2.it_rate*100))%**

vat: pre: **$(fc(sys1.vat_rate*100))%** post: **$(fc(sys2.vat_rate*100))%** 

extend vat? pre: **$(sys1.extend_vat)** post: **$(sys2.extend_vat)**

payroll tax: pre: **$(fc(sys1.ni_rate*100))%** post: **$(fc(sys2.ni_rate*100))%**

benefit rate pre: **£$(fc(sys1.benefit))p.w.** post **£$(fc(sys2.benefit))p.w.**


"""
end
showchanges(sys1, sys2)
end
...
Plots = "~1.39.0"
PlutoUI = "~0.7.54"
PovertyAndInequalityMeasures = "~1.0.10"
StatsPlots = "~0.15.6"
"""
...
manifest_format = "2.0"
project_hash = "9e2b68bbbd8a0169f370f55f6d730b97d2528768"
...
[[deps.PovertyAndInequalityMeasures]]
deps = ["CSV", "DataFrames", "IterableTables", "IteratorInterfaceExtensions", "TableTraits", "TableTraitsUtils"]
git-tree-sha1 = "bb37e1a43eb7c52d6a12cfe29c60b518bda742aa"
uuid = "582645c0-90fa-497a-98b9-936b80116c29"
version = "1.0.10"
