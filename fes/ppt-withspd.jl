...
begin
	settings = Settings() 
	settings.run_name="PPT - final with SPD"
...
begin
	...
	# Uprate CT in baseline to 26/27 levels using inflation
	for i in keys(sys1.loctax.ct.band_d)
        sys1.loctax.ct.band_d[i] *= 1.034
    end

	# set up ppt system in counterfactual
	sys1.loctax.ct.abolished = false
	sys1.loctax.ppt.abolished = true
	...
	sys2.loctax.ppt.spd_fixed_sum = 299 # 1543 (Scotland Band D average 25/26) * 1.034 (26/27 uprate) * 0.25
	=#

	# PPT reform
	sys2.loctax.ppt.local_bands = [400_000]
	sys2.loctax.ppt.local_rates = [0.75, 1.0]
	
	sys2.loctax.ppt.spd_fixed_sum = true
	sys2.loctax.ppt.single_person_discount = 300# 25% of average Band D is 399, but it's expensive. Costs 136/66M (gross/net) relative to 20%SPD. 300 is closer to revenue neutral.

	
	# weeklyise once all systems ready
	weeklyise!(sys1)
	weeklyise!(sys2)	
end
